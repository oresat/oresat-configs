...
from . import INDENT4, INDENT8, INDENT12, INDENT16, snake_to_camel
from .gen_canopend import make_bitfield_lines, make_enum_lines, write_canopend_od

GEN_CANOPEND = "generate od file for main controller app"
...
def register_subparser(subparsers: Any):
    ...
    parser.add_argument("config", help="path to cards config")
    parser.add_argument(
        "-d", "--dir-path", default=".", help="directory path. default %(default)s"
    )
    parser.set_defaults(func=gen_canopend_master)


def write_canopend_master_od(
    name: str,
    od: ObjectDictionary,
...
def write_canopend_mission_defs(
    ...
    master_name = cards_config.master.name

    for mission_config in mission_configs:
        name_upper = mission_config.name.upper()
        mission_lines = [
            "from ..nodes import Node\n",
            f"from ..{master_name}_od import {snake_to_camel(master_name)}Entry\n" "\n",
        ]
        ...
        for names in mission_config.beacon.fields:
            mission_lines.append(
                f"{INDENT4}{node_name_camel}Entry.{'_'.join(names).upper()},\n"
            )
        mission_lines.append("]\n")
...
def write_canopend_od_all(
    ...
    write_canopend_master_od(
        cards_config.master.name,
        od_db[cards_config.master.name],
        cards_config,
        common_od_configs,
        dir_path,
    )
...
def write_canopend_master(
    ...
    master_name = cards_config.master.name
    write_canopend_mission_defs(master_name, mission_configs, cards_config, dir_path)
    write_canopend_fram_def(cards_config.master, od_configs[master_name].fram, dir_path)
    write_canopend_nodes(cards_config, dir_path)


def gen_canopend_master(args: Namespace):
